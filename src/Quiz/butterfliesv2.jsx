// GuessSync E22 — Guess the Butterfly · v2.1 FULL (cold-open · variable timer · hybrid options · checkpoints · ✓/✗ · facts · bed)
import React from "react";
import { AbsoluteFill, Audio, Img, Sequence, staticFile, interpolate, spring, useCurrentFrame, useVideoConfig } from "remotion";
import { BUTTERFLIES as LOGOS } from "./butterfliesData";
import { BUTTERFLY_FACTS as FACTS } from "./butterflyFacts";
...
// تايمر متغيّر حسب المستوى (فريمات حتى الكشف) — سهل 4ث / متوسط 5ث / صعب 7ث / مستحيل 7ث
const TIER_REVEAL = { easy: 120, medium: 150, hard: 210, impossible: 210 };
const HOLD = 108;                       // مدة الكشف بعد التايمر (قراءة الاسم + المعلومة)
const LT = { cold: 250, intro: 165, level: 85, check: 96, outro: 225 };
const ABCD = ["A", "B", "C", "D"];
const hasOptionsFor = (lvl) => lvl === "easy" || lvl === "medium";  // هجين
const roundDur = (lvl) => TIER_REVEAL[lvl] + HOLD;

const makeTicks = (revealAt) => {
  const t = [];
  for (let f = 0; f <= revealAt - 40; f += 30) t.push(f);
  t.push(revealAt - 34, revealAt - 22, revealAt - 12, revealAt - 5);  // dense/accelerating near the end
  return t;
};
...
// شريط التايمر — يمتلئ يسار→يمين ويسخّن للأحمر قرب النهاية · مدّة متغيّرة
...
const SnakeImg = ({ slug, w, h, revealed, revealAt }) => {
  const frame = useCurrentFrame();
  // Ken Burns: زوم بطيء أثناء السؤال ثم استقرار عند الكشف (كسر الجمود)
  ...
  return <Img src={staticFile(`butterflies/${slug}.jpg`)} style={{ width: w - 10, height: h - 10, objectFit: "cover", borderRadius: 22, transform: `scale(${settle})` }} />;
};
...
const LogoRound = ({ logo, num }) => {
  ...
  const revealAt = TIER_REVEAL[logo.level];
  const revealed = frame >= revealAt;
  ...
  const hasOptions = hasOptionsFor(logo.level);
  ...
  return (
    <AbsoluteFill>
      <TierMeter level={logo.level} />
      <div style={{ position: "absolute", top: 84, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 50, color: "#fff", letterSpacing: 1, textShadow: "0 3px 16px rgba(0,0,0,0.4)" }}>GUESS THE BUTTERFLY</div>
      ...
      <div style={{ position: "absolute", top: hasOptions ? 166 : 150, left: 0, right: 0, display: "flex", justifyContent: "center", alignItems: "flex-start" }}>
        <div style={{ transform: `scale(${interpolate(enter, [0, 1], [0.4, 1]) * pop}) translateY(${interpolate(enter, [0, 1], [60, 0]) + floatY}px) rotate(${rot}deg)`, opacity: enter, width: cardW, height: cardH, borderRadius: 30, background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: revealed ? `0 0 70px ${accent}` : "0 24px 60px rgba(0,0,0,0.5)", border: `5px solid ${revealed ? accent : "rgba(255,255,255,0.3)"}`, overflow: "hidden" }}>
          <SnakeImg slug={logo.slug} w={cardW} h={cardH} revealed={revealed} revealAt={revealAt} />
        </div>
      </div>
      {hasOptions && (
      ...
      {!hasOptions && revealed && (
        <div style={{ position: "absolute", top: 636, left: 0, right: 0, textAlign: "center", transform: `scale(${interpolate(nameS, [0, 1], [0.3, 1])})` }}>
          <div style={{ fontFamily: font, fontWeight: 900, fontSize: logo.name.length > 14 ? 78 : 94, color: "#fff", textShadow: `0 0 42px ${accent}` }}>
            <span style={{ color: "#3BE07A", marginInlineEnd: 16 }}>✓</span>{logo.name}
          </div>
        </div>
      )}
      {revealed && FACTS[logo.slug] && (
        <div style={{ position: "absolute", top: 806, left: 150, right: 150, display: "flex", justifyContent: "center", opacity: interpolate(frame, [revealAt + 12, revealAt + 30], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }), transform: `translateY(${interpolate(frame, [revealAt + 12, revealAt + 30], [16, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" })}px)` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 18, background: "rgba(6,10,32,0.78)", border: `3px solid ${GAME.gold}`, borderRadius: 18, padding: "16px 38px", maxWidth: 1500, boxShadow: "0 12px 34px rgba(0,0,0,0.45)" }}>
            ...
            <span style={{ fontFamily: font, fontWeight: 800, fontSize: 42, color: "#fff", lineHeight: 1.12 }}>{FACTS[logo.slug]}</span>
          </div>
        </div>
      )}
      ...
      <Sequence from={revealAt + 6} durationInFrames={100}><Audio src={staticFile(`sfx/bt-${logo.slug}.wav`)} volume={1} /></Sequence>
    </AbsoluteFill>
  );
};

// COLD OPEN — سؤال صادم أول ~8ث قبل أي براند (يمسك المشاهد)
const ColdOpen = ({ logo }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const rAt = 130;
  ...
  return (
    <AbsoluteFill>
      <div style={{ position: "absolute", top: 92, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 68, color: "#fff", textShadow: "0 3px 18px rgba(0,0,0,0.5)" }}>
        {revealed ? <span><span style={{ color: "#3BE07A" }}>✓ </span>{logo.name}</span> : <span><span style={{ color: GAME.red }}>95%</span> CAN'T NAME THIS</span>}
      </div>
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
        <div style={{ width: 560, height: 470, borderRadius: 30, background: "#fff", overflow: "hidden", border: `6px solid ${revealed ? "#3BE07A" : accent}`, boxShadow: revealed ? "0 0 80px #3BE07A" : "0 24px 60px rgba(0,0,0,0.55)", transform: `scale(${interpolate(s, [0, 1], [0.6, 1])})` }}>
          <Img src={staticFile(`butterflies/${logo.slug}.jpg`)} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        </div>
      </AbsoluteFill>
      ...
      <Sequence from={rAt + 6} durationInFrames={100}><Audio src={staticFile(`sfx/bt-${logo.slug}.wav`)} volume={1} /></Sequence>
    </AbsoluteFill>
  );
};

// CHECKPOINT — عند 25/50/75: حفّز العدّ الذاتي (تفاعل)
const Checkpoint = ({ num }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ fontFamily: font, fontWeight: 800, fontSize: 40, color: "rgba(255,255,255,0.85)" }}>Keep counting — it only gets harder 🦋</div>
      </AbsoluteFill>
    </AbsoluteFill>
  );
};
...
const Intro = () => {
  ...
  return (
    <AbsoluteFill>
      <Audio src={staticFile("sfx/vo-intro-butterfly.wav")} volume={0.95} />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", gap: 26 }}>
        ...
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: 84, color: "#fff", letterSpacing: 2, textAlign: "center", lineHeight: 1.04 }}>GUESS THE<br /><span style={{ color: GAME.gold }}>BUTTERFLY</span></div>
        <div style={{ display: "flex", gap: 16 }}>
          <span style={{ fontFamily: font, fontWeight: 800, fontSize: 34, color: GAME.blueDeep, background: GAME.gold, padding: "8px 26px", borderRadius: 999 }}>100 BUTTERFLIES</span>
...
const build = (logos) => {
  const segs = [];
  let f = 0;
  const coldLogo = logos.find((l) => l.slug === "comet-moth") || logos.find((l) => l.level === "impossible") || logos[0];
  segs.push({ t: "cold", logo: coldLogo, from: f, dur: LT.cold }); f += LT.cold;
  ...
  for (const lg of logos) {
    ...
    num += 1;
    segs.push({ t: "round", logo: lg, num, from: f, dur: roundDur(lg.level) }); f += roundDur(lg.level);
...
export const BUTTERFLIESV2_FRAMES = build(LOGOS).total;
export const ButterfliesV2Quiz = () => <Comp logos={LOGOS} />;
