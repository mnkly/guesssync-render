...
const FlagRound = ({ flag, num }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const accent = LVL[flag.level].accent;
  ...
  return (
    <AbsoluteFill>
      <div style={{ position: "absolute", top: 116, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 56, color: "#fff", letterSpacing: 1, textShadow: "0 3px 16px rgba(0,0,0,0.4)" }}>GUESS THE FLAG</div>
      <Progress num={num} accent={accent} />
      <Ring accent={accent} />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
        <div style={{ transform: `scale(${interpolate(enter, [0, 1], [0.4, 1]) * pop}) translateY(${interpolate(enter, [0, 1], [80, 0]) + floatY}px) rotate(${rot}deg)`, opacity: enter, padding: 12, borderRadius: 20, background: "#fff", boxShadow: revealed ? `0 0 80px ${accent}` : "0 30px 70px rgba(0,0,0,0.5)", border: `5px solid ${revealed ? accent : "rgba(255,255,255,0.3)"}` }}>
          <Img src={staticFile(`flags/${flag.iso}.svg`)} style={{ width: 500, height: 333, objectFit: "cover", borderRadius: 10, display: "block" }} />
        </div>
      </AbsoluteFill>
      ...
      {revealed && (
        <div style={{ position: "absolute", bottom: 78, left: 0, right: 0, textAlign: "center", transform: `scale(${interpolate(nameS, [0, 1], [0.3, 1])})` }}>
          <div style={{ fontFamily: font, fontWeight: 900, fontSize: 88, color: "#fff", textShadow: `0 0 42px ${accent}` }}>
            <span style={{ color: accent, marginInlineEnd: 16 }}>✓</span>{flag.name}
          </div>
          <div style={{ height: 8, width: interpolate(nameS, [0, 1], [0, 380]), background: accent, borderRadius: 999, margin: "14px auto 0", boxShadow: `0 0 20px ${accent}` }} />
        </div>
      )}
      ...
      {HAS_NAME_VO && (
        <Sequence from={LT.reveal + 6} durationInFrames={70}><Audio src={staticFile(`sfx/fl-${flag.iso}.wav`)} volume={1} /></Sequence>
      )}
    </AbsoluteFill>
  );
};
...
const Intro = () => {
  ...
  return (
    <AbsoluteFill>
      <Audio src={staticFile("sfx/vo-intro-flag.wav")} volume={0.95} />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", gap: 26 }}>
        ...
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: 84, color: "#fff", letterSpacing: 2 }}>GUESS THE <span style={{ color: GAME.gold }}>FLAG</span></div>
...
const build = (flags) => {
  ...
  for (const fl of flags) {
    if (fl.level !== last) { segs.push({ t: "level", level: fl.level, from: f, dur: LT.level }); f += LT.level; last = fl.level; }
    num += 1;
    segs.push({ t: "round", flag: fl, num, from: f, dur: LT.round }); f += LT.round;
  }
    ...
    <AbsoluteFill style={{ backgroundColor: GAME.blueDeep }}>
      ...
      {segs.map((s, i) => (
        <Sequence key={i} from={s.from} durationInFrames={s.dur}>
          ...
          {s.t === "round" && <FlagRound flag={s.flag} num={s.num} />}
          {s.t === "outro" && <Outro />}
        </Sequence>
      ))}
    </AbsoluteFill>
...
export const FLAG2_FRAMES = build(FLAGS2).total;
export const FLAG2_SAMPLE_FRAMES = build(FLAGS2.slice(0, 6)).total;
export const FlagQuiz2GS = () => <Comp flags={FLAGS2} />;
export const FlagQuiz2GSSample = () => <Comp flags={FLAGS2.slice(0, 6)} />;
