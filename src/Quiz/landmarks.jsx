// «Guess the Country by its Landmark» — GuessSync · E16 · 100 معلم (25/مستوى)
// صور معالم Higgsfield (landmarks/<iso>.png) · يكشف اسم الدولة · يعيد استخدام صوت fl-<iso>.wav (صفر صوت جديد)
import React from "react";
import { AbsoluteFill, Audio, Img, Sequence, staticFile, interpolate, spring, useCurrentFrame, useVideoConfig } from "remotion";
import { LANDMARKS as LOGOS } from "./landmarksData";
...
const LogoRound = ({ logo, num }) => {
  ...
  const rot = interpolate(enter, [0, 1], [-14, 0]);
  const floatY = Math.sin(frame / 20) * 8;
  const pop = interpolate(frame, [LT.reveal, LT.reveal + 6, LT.reveal + 20], [1, 1.14, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  return (
    <AbsoluteFill>
      <div style={{ position: "absolute", top: 116, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 56, color: "#fff", letterSpacing: 1, textShadow: "0 3px 16px rgba(0,0,0,0.4)" }}>WHICH COUNTRY IS THIS?</div>
      <Progress num={num} accent={accent} />
      <Ring accent={accent} />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
        <div style={{ transform: `scale(${interpolate(enter, [0, 1], [0.4, 1]) * pop}) translateY(${interpolate(enter, [0, 1], [80, 0]) + floatY}px) rotate(${rot}deg)`, opacity: enter, width: 460, height: 360, borderRadius: 34, background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: revealed ? `0 0 80px ${accent}` : "0 30px 70px rgba(0,0,0,0.5)", border: `5px solid ${revealed ? accent : "rgba(255,255,255,0.3)"}`, overflow: "hidden" }}>
          <Img src={staticFile(`landmarks/${logo.key}.png`)} style={{ width: 450, height: 350, objectFit: "cover", borderRadius: 24 }} />
        </div>
      </AbsoluteFill>
      ...
      {revealed && (
        <div style={{ position: "absolute", bottom: 78, left: 0, right: 0, textAlign: "center", transform: `scale(${interpolate(nameS, [0, 1], [0.3, 1])})` }}>
          <div style={{ fontFamily: font, fontWeight: 900, fontSize: logo.country.length > 16 ? 74 : 96, color: "#fff", textShadow: `0 0 42px ${accent}` }}>
            <span style={{ color: accent, marginInlineEnd: 16 }}>✓</span>{logo.country}
          </div>
          <div style={{ fontFamily: font, fontWeight: 800, fontSize: 38, color: "rgba(255,255,255,0.85)", marginTop: 4 }}>{logo.landmark}</div>
          <div style={{ height: 8, width: interpolate(nameS, [0, 1], [0, 360]), background: accent, borderRadius: 999, margin: "12px auto 0", boxShadow: `0 0 20px ${accent}` }} />
        </div>
      )}
      ...
      {HAS_NAME_VO && (
        <Sequence from={LT.reveal + 6} durationInFrames={70}><Audio src={staticFile(`sfx/fl-${logo.iso}.wav`)} volume={1} /></Sequence>
      )}
    </AbsoluteFill>
  );
};
...
const LevelIntro = ({ level }) => {
  ...
  return (
    <AbsoluteFill>
      <Audio src={staticFile(`sfx/vo-${level}.wav`)} volume={0.95} />
...
const Intro = () => {
  ...
  return (
    <AbsoluteFill>
      <Audio src={staticFile("sfx/vo-intro-country.wav")} volume={0.95} />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", gap: 24 }}>
        <Img src={staticFile("brand/owl-cheer.png")} style={{ width: 240, height: 240, objectFit: "contain", transform: `scale(${interpolate(s, [0, 1], [0.5, 1])}) translateY(${Math.sin(frame / 12) * 10}px)`, filter: "drop-shadow(0 14px 30px rgba(0,0,0,0.5))" }} />
        ...
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: 78, color: "#fff", letterSpacing: 2 }}>GUESS THE <span style={{ color: GAME.gold }}>COUNTRY</span></div>
        <div style={{ fontFamily: font, fontWeight: 800, fontSize: 44, color: "rgba(255,255,255,0.85)", letterSpacing: 4 }}>BY ITS LANDMARK</div>
        <div style={{ display: "flex", gap: 16 }}>
          <span style={{ fontFamily: font, fontWeight: 800, fontSize: 34, color: GAME.blueDeep, background: GAME.gold, padding: "8px 26px", borderRadius: 999 }}>100 LANDMARKS</span>
...
const Outro = () => {
  ...
  return (
    <AbsoluteFill>
      <Audio src={staticFile("sfx/vo-outro.wav")} volume={0.95} />
...
export const LANDMARK_FRAMES = build(LOGOS).total;
export const LandmarkQuiz = () => <Comp logos={LOGOS} />;
