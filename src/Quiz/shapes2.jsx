...
import { SHAPES2 as FLAGS } from "./shapes2Data";
...
const FlagRound = ({ flag, num }) => {
  ...
  return (
    <AbsoluteFill>
      <div style={{ position: "absolute", top: 116, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 56, color: "#fff", letterSpacing: 1, textShadow: "0 3px 16px rgba(0,0,0,0.4)" }}>WHAT COUNTRY IS THIS?</div>
      <Progress num={num} accent={accent} />
      <Ring accent={accent} />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
        <div style={{
          transform: `scale(${interpolate(enter, [0, 1], [0.4, 1]) * pop}) translateY(${interpolate(enter, [0, 1], [80, 0]) + floatY}px)`, opacity: enter,
          width: 680, height: 500,
          WebkitMaskImage: `url(${staticFile(`maps/${flag.iso}.svg`)})`, maskImage: `url(${staticFile(`maps/${flag.iso}.svg`)})`,
          ...
          background: revealed ? `linear-gradient(160deg, #FFE888, ${accent})` : "linear-gradient(160deg, #ffffff, #b9cdf0)",
          filter: revealed ? `drop-shadow(0 0 45px ${accent})` : "drop-shadow(0 12px 32px rgba(0,0,0,0.7))",
        }} />
      </AbsoluteFill>
  ...
  return (
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", gap: 26 }}>
        ...
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: 84, color: "#fff", letterSpacing: 2 }}>GUESS THE <span style={{ color: GAME.gold }}>COUNTRY</span></div>
...
export const SHAPE2_FRAMES = build(FLAGS).total;
export const ShapeQuiz2GS = () => <Comp flags={FLAGS} />;
