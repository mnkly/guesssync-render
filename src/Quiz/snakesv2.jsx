// GuessSync E23 — Guess the Snake · v2.1 FULL (cold-open · variable timer · hybrid options · checkpoints · ✓/✗ · facts · bed)
import React from "react";
import { AbsoluteFill, Audio, Img, Sequence, staticFile, interpolate, spring, useCurrentFrame, useVideoConfig } from "remotion";
import { SNAKES as LOGOS } from "./snakesData";
import { SNAKE_FACTS as FACTS } from "./snakeFacts";
...
const SnakeImg = ({ slug, w, h, revealed, revealAt }) => {
  ...
  return <Img src={staticFile(`snakes/${slug}.jpg`)} style={{ width: w - 10, height: h - 10, objectFit: "cover", borderRadius: 22, transform: `scale(${settle})` }} />;
};
...
const LogoRound = ({ logo, num }) => {
  ...
  return (
    <AbsoluteFill>
      ...
      <div style={{ position: "absolute", top: 84, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 50, color: "#fff", letterSpacing: 1, textShadow: "0 3px 16px rgba(0,0,0,0.4)" }}>GUESS THE SNAKE</div>
      ...
      <Sequence from={revealAt + 6} durationInFrames={100}><Audio src={staticFile(`sfx/sn-${logo.slug}.wav`)} volume={1} /></Sequence>
    </AbsoluteFill>
  );
};
    ...
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
        <div style={{ width: 560, height: 470, borderRadius: 30, background: "#fff", overflow: "hidden", border: `6px solid ${revealed ? "#3BE07A" : accent}`, boxShadow: revealed ? "0 0 80px #3BE07A" : "0 24px 60px rgba(0,0,0,0.55)", transform: `scale(${interpolate(s, [0, 1], [0.6, 1])})` }}>
          <Img src={staticFile(`snakes/${logo.slug}.jpg`)} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        </div>
      </AbsoluteFill>
      ...
      <Sequence from={rAt + 6} durationInFrames={100}><Audio src={staticFile(`sfx/sn-${logo.slug}.wav`)} volume={1} /></Sequence>
    </AbsoluteFill>
  );
};
...
const Intro = () => {
  ...
  return (
    <AbsoluteFill>
      <Audio src={staticFile("sfx/vo-intro-snake.wav")} volume={0.95} />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", gap: 26 }}>
        ...
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: 84, color: "#fff", letterSpacing: 2, textAlign: "center", lineHeight: 1.04 }}>GUESS THE<br /><span style={{ color: GAME.gold }}>SNAKE</span></div>
        <div style={{ display: "flex", gap: 16 }}>
          <span style={{ fontFamily: font, fontWeight: 800, fontSize: 34, color: GAME.blueDeep, background: GAME.gold, padding: "8px 26px", borderRadius: 999 }}>100 SNAKES</span>
...
const build = (logos) => {
  const segs = [];
  let f = 0;
  const coldLogo = logos.find((l) => l.slug === "spider-tailed-horned-viper") || logos.find((l) => l.level === "impossible") || logos[0];
...
export const SNAKESV2_FRAMES = build(LOGOS).total;
export const SnakesV2Quiz = () => <Comp logos={LOGOS} />;
