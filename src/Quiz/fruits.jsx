...
const LogoRound = ({ logo, num }) => {
  ...
  return (
    <AbsoluteFill>
      <div style={{ position: "absolute", top: 116, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 56, color: "#fff", letterSpacing: 1, textShadow: "0 3px 16px rgba(0,0,0,0.4)" }}>GUESS THE FRUIT OR VEG</div>
      ...
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
        <div style={{ transform: `scale(${interpolate(enter, [0, 1], [0.4, 1]) * pop}) translateY(${interpolate(enter, [0, 1], [80, 0]) + floatY}px) rotate(${rot}deg)`, opacity: enter, width: 480, height: 440, borderRadius: 34, background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: revealed ? `0 0 80px ${accent}` : "0 30px 70px rgba(0,0,0,0.5)", border: `5px solid ${revealed ? accent : "rgba(255,255,255,0.3)"}`, overflow: "hidden" }}>
          <Img src={staticFile(`fruits/${logo.slug}.jpg`)} style={{ width: 470, height: 430, objectFit: "cover", borderRadius: 24 }} />
        </div>
      </AbsoluteFill>
...
export const FRUITS_FRAMES = build(LOGOS).total;
export const FruitsQuiz = () => <Comp logos={LOGOS} />;
