// «Guess the Painting» — GuessSync · E18 · 100 لوحة (25/مستوى) · صور Public Domain من ويكيبيديا (مجانية)
// بطاقة إطار (objectFit contain لعرض اللوحة كاملة) · يكشف العنوان + الفنان · صوت pt-<slug>.wav
import React from "react";
import { AbsoluteFill, Audio, Img, Sequence, staticFile, interpolate, spring, useCurrentFrame, useVideoConfig } from "remotion";
import { PAINTINGS as LOGOS } from "./paintingsData";
...
const LogoRound = ({ logo, num }) => {
  ...
  const rot = interpolate(enter, [0, 1], [-8, 0]);
  const floatY = Math.sin(frame / 20) * 6;
  const pop = interpolate(frame, [LT.reveal, LT.reveal + 6, LT.reveal + 20], [1, 1.1, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  return (
    <AbsoluteFill>
      <div style={{ position: "absolute", top: 104, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 56, color: "#fff", letterSpacing: 1, textShadow: "0 3px 16px rgba(0,0,0,0.4)" }}>GUESS THE PAINTING</div>
      <Progress num={num} accent={accent} />
      <Ring accent={accent} />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", top: 20 }}>
        <div style={{ transform: `scale(${interpolate(enter, [0, 1], [0.5, 1]) * pop}) translateY(${interpolate(enter, [0, 1], [70, 0]) + floatY}px) rotate(${rot}deg)`, opacity: enter, padding: 18, borderRadius: 10, background: "linear-gradient(160deg, #fdfbf4, #efe7d2)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: revealed ? `0 0 80px ${accent}` : "0 30px 70px rgba(0,0,0,0.55)", border: `12px solid ${revealed ? accent : "#2a2118"}` }}>
          <Img src={staticFile(`paintings/${logo.slug}.jpg`)} style={{ maxWidth: 520, maxHeight: 560, width: "auto", height: "auto", objectFit: "contain", display: "block" }} />
        </div>
      </AbsoluteFill>
      <AbsoluteFill style={{ background: "#fff", opacity: flash, pointerEvents: "none" }} />
      <Confetti accent={accent} />
      {revealed && (
        <div style={{ position: "absolute", bottom: 60, left: 0, right: 0, textAlign: "center", transform: `scale(${interpolate(nameS, [0, 1], [0.4, 1])})`, padding: "0 40px" }}>
          <div style={{ fontFamily: font, fontWeight: 900, fontSize: logo.title.length > 22 ? 58 : (logo.title.length > 15 ? 72 : 90), color: "#fff", textShadow: `0 0 42px ${accent}` }}>
            <span style={{ color: accent, marginInlineEnd: 14 }}>✓</span>{logo.title}
          </div>
          <div style={{ fontFamily: font, fontWeight: 800, fontSize: 40, color: "rgba(255,255,255,0.85)", marginTop: 6 }}>{logo.artist}</div>
          <div style={{ height: 8, width: interpolate(nameS, [0, 1], [0, 340]), background: accent, borderRadius: 999, margin: "12px auto 0", boxShadow: `0 0 20px ${accent}` }} />
        </div>
      )}
      ...
      {HAS_NAME_VO && (
        <Sequence from={LT.reveal + 6} durationInFrames={70}><Audio src={staticFile(`sfx/pt-${logo.slug}.wav`)} volume={1} /></Sequence>
      )}
    </AbsoluteFill>
  );
};
...
const Intro = () => {
  ...
  return (
    <AbsoluteFill>
      <Audio src={staticFile("sfx/vo-intro-painting.wav")} volume={0.95} />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", gap: 26 }}>
        ...
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: 80, color: "#fff", letterSpacing: 2 }}>GUESS THE <span style={{ color: GAME.gold }}>PAINTING</span></div>
        <div style={{ display: "flex", gap: 16 }}>
          <span style={{ fontFamily: font, fontWeight: 800, fontSize: 34, color: GAME.blueDeep, background: GAME.gold, padding: "8px 26px", borderRadius: 999 }}>100 PAINTINGS</span>
...
export const PAINTING_FRAMES = build(LOGOS).total;
export const PaintingQuiz = () => <Comp logos={LOGOS} />;
