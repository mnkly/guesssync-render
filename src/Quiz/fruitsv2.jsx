...
import { FRUITS as LOGOS } from "./fruitsData";
...
const LogoRound = ({ logo, num }) => {
  ...
  return (
    <AbsoluteFill>
      <div style={{ position: "absolute", top: 82, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 54, color: "#fff", letterSpacing: 1, textShadow: "0 3px 16px rgba(0,0,0,0.4)" }}>GUESS THE FRUIT OR VEG</div>
      ...
      <div style={{ position: "absolute", top: hasOptions ? 166 : 0, left: 0, right: 0, bottom: hasOptions ? undefined : 0, display: "flex", justifyContent: "center", alignItems: hasOptions ? "flex-start" : "center" }}>
        <div style={{ transform: `scale(${interpolate(enter, [0, 1], [0.4, 1]) * pop}) translateY(${interpolate(enter, [0, 1], [60, 0]) + floatY}px) rotate(${rot}deg)`, opacity: enter, width: cardW, height: cardH, borderRadius: 30, background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: revealed ? `0 0 70px ${accent}` : "0 24px 60px rgba(0,0,0,0.5)", border: `5px solid ${revealed ? accent : "rgba(255,255,255,0.3)"}`, overflow: "hidden" }}>
          <Img src={staticFile(`fruits/${logo.slug}.jpg`)} style={{ width: cardW - 10, height: cardH - 10, objectFit: "cover", borderRadius: 22 }} />
        </div>
      </div>
      {hasOptions && (
      ...
      {HAS_NAME_VO && (
        <Sequence from={LT.reveal + 6} durationInFrames={70}><Audio src={staticFile(`sfx/fr-${logo.slug}.wav`)} volume={1} /></Sequence>
      )}
    </AbsoluteFill>
  );
};
...
const Intro = () => {
  ...
  return (
    <AbsoluteFill>
      <Audio src={staticFile("sfx/vo-intro-fruit.wav")} volume={0.95} />
...
export const FRUITSV2_FRAMES = build(LOGOS).total;
export const FruitsV2Quiz = () => <Comp logos={LOGOS} />;
