// «Guess the Country from its Capital» — GuessSync · E04 · يعيد استخدام الأعلام + fl-<iso>.wav
import React from "react";
import { AbsoluteFill, Audio, Img, Sequence, staticFile, interpolate, spring, useCurrentFrame, useVideoConfig } from "remotion";
import { CAPITALS2 as CAPITALS } from "./capitals2Data";
...
const CountryRound = ({ item, num }) => {
  ...
  return (
    <AbsoluteFill>
      <div style={{ position: "absolute", top: 108, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 52, color: "#fff", letterSpacing: 1, textShadow: "0 3px 16px rgba(0,0,0,0.4)" }}>WHICH COUNTRY'S CAPITAL?</div>
      <Progress num={num} accent={accent} />
      <Ring accent={accent} />
      {/* السؤال: اسم العاصمة كبير */}
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", transform: "translateY(-40px)" }}>
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: 128, color: GAME.gold, textShadow: `0 6px 40px rgba(255,197,61,0.5)`, transform: `scale(${interpolate(enter, [0, 1], [0.5, 1])})`, opacity: enter, textAlign: "center", padding: "0 60px" }}>{item.capital}</div>
      </AbsoluteFill>
      <AbsoluteFill style={{ background: "#fff", opacity: flash, pointerEvents: "none" }} />
      <Confetti accent={accent} />
      {revealed && (
        <div style={{ position: "absolute", bottom: 60, left: 0, right: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 16, transform: `scale(${interpolate(nameS, [0, 1], [0.4, 1]) * pop})` }}>
          <Img src={staticFile(`flags/${item.iso}.svg`)} style={{ width: 200, height: 133, objectFit: "cover", borderRadius: 8, border: `4px solid ${accent}`, boxShadow: `0 0 50px ${accent}` }} />
          <div style={{ fontFamily: font, fontWeight: 900, fontSize: 80, color: "#fff", textShadow: `0 0 42px ${accent}` }}>
            <span style={{ color: accent, marginInlineEnd: 16 }}>✓</span>{item.country}
          </div>
        </div>
      )}
      ...
      {HAS_NAME_VO && (
        <Sequence from={LT.reveal + 6} durationInFrames={70}><Audio src={staticFile(`sfx/fl-${item.iso}.wav`)} volume={1} /></Sequence>
      )}
    </AbsoluteFill>
  );
};
  ...
  return (
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", gap: 26 }}>
        ...
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: 76, color: "#fff", letterSpacing: 2, textAlign: "center" }}>GUESS THE <span style={{ color: GAME.gold }}>COUNTRY</span></div>
        <div style={{ display: "flex", gap: 16 }}>
          <span style={{ fontFamily: font, fontWeight: 800, fontSize: 34, color: GAME.blueDeep, background: GAME.gold, padding: "8px 26px", borderRadius: 999 }}>FROM ITS CAPITAL</span>
          <span style={{ fontFamily: font, fontWeight: 800, fontSize: 34, color: "#fff", border: `2px solid ${GAME.gold}`, padding: "8px 26px", borderRadius: 999 }}>3 SECONDS</span>
        </div>
      </AbsoluteFill>
    </AbsoluteFill>
  );
    ...
    <AbsoluteFill style={{ backgroundColor: GAME.blueDeep }}>
      ...
      {segs.map((s, i) => (
        <Sequence key={i} from={s.from} durationInFrames={s.dur}>
          ...
          {s.t === "round" && <CountryRound item={s.item} num={s.num} />}
          {s.t === "outro" && <Outro />}
        </Sequence>
      ))}
    </AbsoluteFill>
...
export const COUNTRY2_FRAMES = build(CAPITALS).total;
export const CountryQuiz2GS = () => <Comp items={CAPITALS} />;
