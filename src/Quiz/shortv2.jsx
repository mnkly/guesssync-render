...
const Clue = ({ item, mode, revealed, accent, zoom }) => {
  ...
  // scrambled (E115): pure-text clue — no image. Renders item.scrambled as big bold letter-spaced
  // text inside the same white card used by every other mode. GUARD: only mode === "scrambled"
  // hits this branch; every existing image mode below is untouched.
  if (mode === "scrambled") {
    const len = String(item.scrambled || "").length;
    const fs = Math.max(48, Math.min(120, Math.floor(920 / Math.max(len, 3))));
    return (
      <div style={{ ...card, width: 900, height: 420, padding: 30 }}>
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: fs, letterSpacing: Math.max(2, fs * 0.09), color: GAME.blueDeep, textAlign: "center", lineHeight: 1.1, wordBreak: "break-word" }}>{item.scrambled}</div>
      </div>
    );
  }
  if (mode === "flagzoom") {
