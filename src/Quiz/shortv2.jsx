...
const QWORD = { movies: "GUESS THE MOVIE", efoods: "GUESS THE FOOD", esongs: "GUESS THE SONG", egames: "GUESS THE GAME", ecountries: "GUESS THE COUNTRY", etv: "GUESS THE TV SHOW", ebrands: "GUESS THE BRAND", ebooks: "GUESS THE BOOK", gaming: "GUESS THE GAME", airlines: "GUESS THE AIRLINE", fastfood: "GUESS THE FAST FOOD", fish: "GUESS THE FISH", fish2: "GUESS THE FISH", cocktails: "GUESS THE COCKTAIL", breads: "GUESS THE BREAD", nuts: "GUESS THE NUT OR SEED", cacti: "GUESS THE CACTUS OR SUCCULENT", desserts: "GUESS THE DESSERT", kitchen: "GUESS THE KITCHEN UTENSIL", spices: "GUESS THE HERB OR SPICE", pasta: "GUESS THE PASTA", cheese: "GUESS THE CHEESE", trains: "GUESS THE TRAIN", primates: "GUESS THE PRIMATE", boats: "GUESS THE BOAT", shells: "GUESS THE SEASHELL", mushrooms: "GUESS THE MUSHROOM", aircraft: "GUESS THE AIRCRAFT", space: "GUESS THE SPACE OBJECT", sports: "GUESS THE SPORT", trees: "GUESS THE TREE", tools: "GUESS THE TOOL", rocks: "GUESS THE ROCK", reptiles: "GUESS THE REPTILE", animals2: "GUESS THE ANIMAL", animals3: "GUESS THE ANIMAL", landmarks2: "GUESS THE LANDMARK", spiders: "GUESS THE SPIDER", amphibians: "GUESS THE AMPHIBIAN", horses: "GUESS THE HORSE BREED", gems: "GUESS THE GEMSTONE", insects: "GUESS THE INSECT", insects2: "GUESS THE INSECT", cats: "GUESS THE CAT BREED", carlogos: "GUESS THE CAR", logos: "GUESS THE LOGO", clubs: "GUESS THE CLUB", clubbadges: "GUESS THE FOOTBALL CLUB", animals: "GUESS THE ANIMAL", foods: "GUESS THE FOOD", foods2: "GUESS THE DISH", dogs: "GUESS THE DOG BREED", dogs2: "GUESS THE DOG BREED", cars: "GUESS THE CAR", cars2: "GUESS THE CAR", birds: "GUESS THE BIRD", birds2: "GUESS THE BIRD", sea: "GUESS THE SEA CREATURE", fruits: "FRUIT OR VEG?", flowers: "GUESS THE FLOWER", flowers2: "GUESS THE FLOWER", butterflies: "GUESS THE BUTTERFLY", snakes: "GUESS THE SNAKE", dinos: "GUESS THE DINOSAUR", paintings: "GUESS THE PAINTING", landmarks: "WHICH COUNTRY?", landmarkzoom: "GUESS THE LANDMARK", foodzoom: "GUESS THE FOOD", flags: "GUESS THE COUNTRY", flagzoom: "GUESS THE ZOOMED FLAG", capitals: "GUESS THE CAPITAL", countries: "WHICH COUNTRY?", shapes: "WHAT COUNTRY?", countryshapes: "GUESS THE COUNTRY", instruments: "GUESS THE INSTRUMENT" };
const voFile = (item, mode) => mode === "movies" ? `sfx/em-${item.id}.wav` : mode === "efoods" ? `sfx/ef-${item.id}.wav` : mode === "esongs" ? `sfx/es-${item.id}.wav` : mode === "egames" ? `sfx/eg-${item.id}.wav` : mode === "ecountries" ? `sfx/ec-${item.id}.wav` : mode === "etv" ? `sfx/et-${item.id}.wav` : mode === "ebrands" ? `sfx/eb-${item.id}.wav` : mode === "ebooks" ? `sfx/bk-${item.id}.wav` : mode === "carlogos" ? `sfx/cl-${item.slug}.wav` : mode === "gaming" ? `sfx/gm-${item.slug}.wav` : mode === "airlines" ? `sfx/al-${item.slug}.wav` : mode === "fastfood" ? `sfx/ff-${item.slug}.wav` : mode === "countryshapes" ? `sfx/cs-${item.slug}.wav` : mode === "instruments" ? `sfx/mi-${item.slug}.wav` : mode === "fish" ? `sfx/fh-${item.slug}.wav` : mode === "fish2" ? `sfx/fs-${item.slug}.wav` : mode === "cocktails" ? `sfx/ck-${item.slug}.wav` : mode === "breads" ? `sfx/br-${item.slug}.wav` : mode === "nuts" ? `sfx/nt-${item.slug}.wav` : mode === "cacti" ? `sfx/cx-${item.slug}.wav` : mode === "desserts" ? `sfx/ds-${item.slug}.wav` : mode === "kitchen" ? `sfx/kt-${item.slug}.wav` : mode === "spices" ? `sfx/sc-${item.slug}.wav` : mode === "pasta" ? `sfx/pa-${item.slug}.wav` : mode === "cheese" ? `sfx/ch-${item.slug}.wav` : mode === "trains" ? `sfx/tn-${item.slug}.wav` : mode === "primates" ? `sfx/pr-${item.slug}.wav` : mode === "boats" ? `sfx/bt-${item.slug}.wav` : mode === "shells" ? `sfx/sh-${item.slug}.wav` : mode === "mushrooms" ? `sfx/mu-${item.slug}.wav` : mode === "aircraft" ? `sfx/ac-${item.slug}.wav` : mode === "space" ? `sfx/sp-${item.slug}.wav` : mode === "sports" ? `sfx/sr-${item.slug}.wav` : mode === "trees" ? `sfx/tr-${item.slug}.wav` : mode === "tools" ? `sfx/tl-${item.slug}.wav` : mode === "rocks" ? `sfx/rk-${item.slug}.wav` : mode === "reptiles" ? `sfx/rp-${item.slug}.wav` : mode === "animals2" ? `sfx/a2-${item.slug}.wav` : mode === "animals3" ? `sfx/a3-${item.slug}.wav` : mode === "landmarks2" ? `sfx/l2-${item.slug}.wav` : mode === "spiders" ? `sfx/sd-${item.slug}.wav` : mode === "amphibians" ? `sfx/am-${item.slug}.wav` : mode === "horses" ? `sfx/hr-${item.slug}.wav` : mode === "gems" ? `sfx/gem-${item.slug}.wav` : mode === "insects" ? `sfx/in-${item.slug}.wav` : mode === "insects2" ? `sfx/i2-${item.slug}.wav` : mode === "cats" ? `sfx/cat-${item.slug}.wav` : mode === "clubs" ? `sfx/cb-${item.slug}.wav` : mode === "clubbadges" ? `sfx/fc-${item.slug}.wav` : mode === "logos" ? `sfx/nm-${item.slug}.wav` : mode === "animals" ? `sfx/an-${item.slug}.wav` : mode === "foods2" ? `sfx/f2-${item.slug}.wav` : mode === "foods" ? `sfx/fd-${item.slug}.wav` : mode === "dogs" ? `sfx/dog-${item.slug}.wav` : mode === "dogs2" ? `sfx/d2-${item.slug}.wav` : mode === "cars" ? `sfx/cm-${item.slug}.wav` : mode === "cars2" ? `sfx/c2-${item.slug}.wav` : mode === "birds" ? `sfx/bd-${item.slug}.wav` : mode === "birds2" ? `sfx/b2-${item.slug}.wav` : mode === "sea" ? `sfx/sc-${item.slug}.wav` : mode === "fruits" ? `sfx/fr-${item.slug}.wav` : mode === "flowers" ? `sfx/fl-${item.slug}.wav` : mode === "flowers2" ? `sfx/w2-${item.slug}.wav` : mode === "butterflies" ? `sfx/bt-${item.slug}.wav` : mode === "snakes" ? `sfx/sn-${item.slug}.wav` : mode === "dinos" ? `sfx/dn-${item.slug}.wav` : mode === "paintings" ? `sfx/pt-${item.slug}.wav` : mode === "flagzoom" ? `sfx/fz-${item.slug}.wav` : mode === "landmarkzoom" ? `sfx/lz-${item.slug}.wav` : mode === "foodzoom" ? `sfx/fz2-${item.slug}.wav` : mode === "capitals" ? `sfx/cp-${item.iso}.wav` : `sfx/fl-${item.iso}.wav`;
const answer = (item, mode) => (mode === "paintings" || mode === "movies" || mode === "efoods" || mode === "esongs" || mode === "egames" || mode === "ecountries" || mode === "etv" || mode === "ebrands" || mode === "ebooks") ? item.title : (mode === "logos" || mode === "clubs" || mode === "clubbadges" || mode === "animals" || mode === "foods" || mode === "foods2" || mode === "dogs" || mode === "dogs2" || mode === "cars" || mode === "cars2" || mode === "birds" || mode === "birds2" || mode === "sea" || mode === "fruits" || mode === "flowers" || mode === "flowers2" || mode === "butterflies" || mode === "snakes" || mode === "dinos" || mode === "carlogos" || mode === "gaming" || mode === "airlines" || mode === "fastfood" || mode === "fish" || mode === "fish2" || mode === "cocktails" || mode === "breads" || mode === "nuts" || mode === "cacti" || mode === "desserts" || mode === "kitchen" || mode === "spices" || mode === "pasta" || mode === "cheese" || mode === "trains" || mode === "primates" || mode === "boats" || mode === "shells" || mode === "mushrooms" || mode === "aircraft" || mode === "space" || mode === "sports" || mode === "trees" || mode === "tools" || mode === "rocks" || mode === "reptiles" || mode === "animals2" || mode === "animals3" || mode === "landmarks2" || mode === "spiders" || mode === "amphibians" || mode === "horses" || mode === "gems" || mode === "insects" || mode === "insects2" || mode === "cats" || mode === "countryshapes" || mode === "instruments" || mode === "landmarkzoom" || mode === "foodzoom") ? item.name : mode === "capitals" ? item.capital : item.country || item.name;
...
export const ShortV2Quiz = ({ items, mode, title, part = 0, zoom = null }) => {
