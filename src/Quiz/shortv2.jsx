...
const ABCD = ["A", "B", "C", "D"];
const nameOf = (x, mode) => mode === "emojiword" ? x.answer : (mode === "paintings" || mode === "movies" || mode === "efoods" || mode === "esongs" || mode === "egames" || mode === "ecountries" || mode === "ecountries2" || mode === "etv" || mode === "ebrands" || mode === "ebooks" || mode === "esongs2" || mode === "movies2" || mode === "egames2" || mode === "etv2" || mode === "ebooks2" || mode === "ebrands2" || mode === "efoods2" || mode === "eheroes" || mode === "edisney" || mode === "eanime" || mode === "eanimal" || mode === "esports" || mode === "ejobs" || mode === "ecartoon") ? x.title : mode === "capitals" ? x.capital : (x.name || x.country || x.slug);
...
const Clue = ({ item, mode, revealed, accent, zoom }) => {
  ...
  if (mode === "movies" || mode === "efoods" || mode === "esongs" || mode === "egames" || mode === "ecountries" || mode === "ecountries2" || mode === "etv" || mode === "ebrands" || mode === "ebooks" || mode === "esongs2" || mode === "movies2" || mode === "egames2" || mode === "etv2" || mode === "ebooks2" || mode === "ebrands2" || mode === "efoods2" || mode === "eheroes" || mode === "edisney" || mode === "eanime" || mode === "eanimal" || mode === "esports" || mode === "ejobs" || mode === "ecartoon") { const chars = [...item.emojis]; const sz = chars.length <= 2 ? 280 : chars.length === 3 ? 220 : 170; return <div style={{ ...card, width: 640, height: 640, display: "flex", flexWrap: "wrap", gap: 20, alignItems: "center", justifyContent: "center", padding: 36 }}>{chars.map((ch, i) => <Img key={i} src={staticFile(`openmoji/${[...ch].map((c) => c.codePointAt(0).toString(16).toUpperCase()).join("-")}.svg`)} style={{ width: sz, height: sz, objectFit: "contain" }} />)}</div>; }
