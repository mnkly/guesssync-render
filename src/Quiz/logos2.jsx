...
const LT = { intro: 130, level: 85, reveal: 90, round: 180, outro: 190 }; // round 180 → إجمالي +10 دقائق
// تكّات ساعة تتسارع في آخر ثانية للتشويق (مؤثر مسموح — لا موسيقى)
const TICKS = [0, 30, 60, 74, 82, 88];
// نطق أسماء الشعارات عند الكشف — مُفعّل (ملفات sfx/nm-<slug>.wav جاهزة)
const HAS_NAME_VO = true;

// ── خلفية Gameshow متحركة (أداء عالٍ) ──
const GameBg = () => {
...
const LogoRound = ({ logo, num }) => {
  ...
  return (
    <AbsoluteFill>
      <div style={{ position: "absolute", top: 116, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 56, color: "#fff", letterSpacing: 1, textShadow: "0 3px 16px rgba(0,0,0,0.4)" }}>GUESS THE LOGO</div>
      ...
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
        <div style={{ transform: `scale(${interpolate(enter, [0, 1], [0.4, 1]) * pop}) translateY(${interpolate(enter, [0, 1], [80, 0]) + floatY}px) rotate(${rot}deg)`, opacity: enter, width: 360, height: 360, borderRadius: 34, background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: revealed ? `0 0 80px ${accent}` : "0 30px 70px rgba(0,0,0,0.5)", border: `5px solid ${revealed ? accent : "rgba(255,255,255,0.3)"}` }}>
          <Img src={staticFile(`logos/${logo.slug}.svg`)} style={{ width: 210, height: 210, objectFit: "contain", filter: "drop-shadow(0 0 1.2px rgba(0,0,0,0.45))" }} />
        </div>
      </AbsoluteFill>
      <AbsoluteFill style={{ background: "#fff", opacity: flash, pointerEvents: "none" }} />
      <Confetti accent={accent} />
      {revealed && (
        <div style={{ position: "absolute", bottom: 92, left: 0, right: 0, textAlign: "center", transform: `scale(${interpolate(nameS, [0, 1], [0.3, 1])})` }}>
          <div style={{ fontFamily: font, fontWeight: 900, fontSize: 96, color: "#fff", textShadow: `0 0 42px ${accent}` }}>
      ...
      {HAS_NAME_VO && (
        <Sequence from={LT.reveal + 6} durationInFrames={70}><Audio src={staticFile(`sfx/nm-${logo.slug}.wav`)} volume={1} /></Sequence>
      )}
    </AbsoluteFill>
  );
};
...
const Intro = () => {
  ...
  return (
    <AbsoluteFill>
      <Audio src={staticFile("sfx/vo-intro.wav")} volume={0.95} />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", gap: 26 }}>
        ...
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: 84, color: "#fff", letterSpacing: 2 }}>GUESS THE <span style={{ color: GAME.gold }}>LOGO</span></div>
        <div style={{ display: "flex", gap: 16 }}>
          <span style={{ fontFamily: font, fontWeight: 800, fontSize: 34, color: GAME.blueDeep, background: GAME.gold, padding: "8px 26px", borderRadius: 999 }}>100 BRANDS</span>
...
const Comp = ({ logos }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: GAME.blueDeep }}>
      <GameBg />
      {/* لا موسيقى — مؤثرات صوتية مسموحة فقط (تكّات + نغمة + تعليق) */}
      <Watermark />
      {segs.map((s, i) => (
...
export const LOGO2_FRAMES = build(LOGOS2).total;
export const LOGO2_SAMPLE_FRAMES = build(LOGOS2.slice(0, 6)).total;
export const LogoQuiz2 = () => <Comp logos={LOGOS2} />;
export const LogoQuiz2Sample = () => <Comp logos={LOGOS2.slice(0, 6)} />;
