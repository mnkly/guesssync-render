...
const OwlReact = ({ revealed }) => {
  ...
  return (
    <Img src={staticFile(revealed ? "brand/owl-cheer.png" : "brand/owl-think.png")}
      style={{ position: "absolute", right: 40, bottom: 24, width: 230, height: 230, objectFit: "contain", transform: `translateY(${bob + jump}px) scale(${revealed ? interpolate(cheer, [0, 1], [0.8, 1.05]) : 1})`, filter: "drop-shadow(0 12px 26px rgba(0,0,0,0.5))" }} />
  );
};
...
const CapitalRound = ({ item, num }) => {
  ...
  return (
    <AbsoluteFill>
      <div style={{ position: "absolute", top: 108, left: 0, right: 0, textAlign: "center", fontFamily: font, fontWeight: 900, fontSize: 52, color: "#fff", letterSpacing: 1, textShadow: "0 3px 16px rgba(0,0,0,0.4)" }}>CAPITAL OF...</div>
      <Progress num={num} accent={accent} />
      <Ring accent={accent} />
      {/* السؤال: علم + اسم الدولة */}
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", flexDirection: "column", gap: 20, transform: `translateY(-30px)` }}>
        <div style={{ transform: `scale(${interpolate(enter, [0, 1], [0.5, 1]) * pop}) translateY(${interpolate(enter, [0, 1], [60, 0]) + floatY}px)`, opacity: enter, padding: 10, borderRadius: 16, background: "#fff", boxShadow: revealed ? `0 0 70px ${accent}` : "0 24px 60px rgba(0,0,0,0.5)", border: `5px solid ${revealed ? accent : "rgba(255,255,255,0.3)"}` }}>
          <Img src={staticFile(`flags/${item.iso}.svg`)} style={{ width: 360, height: 240, objectFit: "cover", borderRadius: 8, display: "block" }} />
        </div>
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: 72, color: "#fff", textShadow: "0 3px 16px rgba(0,0,0,0.5)" }}>{item.country}</div>
      </AbsoluteFill>
      <AbsoluteFill style={{ background: "#fff", opacity: flash, pointerEvents: "none" }} />
      <Confetti accent={accent} />
      {revealed && (
        <div style={{ position: "absolute", bottom: 70, left: 0, right: 0, textAlign: "center", transform: `scale(${interpolate(nameS, [0, 1], [0.3, 1])})` }}>
          <div style={{ fontFamily: font, fontWeight: 900, fontSize: 84, color: "#fff", textShadow: `0 0 42px ${accent}` }}>
            <span style={{ color: accent, marginInlineEnd: 16 }}>✓</span>{item.capital}
          </div>
      ...
      {HAS_NAME_VO && (
        <Sequence from={LT.reveal + 6} durationInFrames={70}><Audio src={staticFile(`sfx/cp-${item.iso}.wav`)} volume={1} /></Sequence>
      )}
    </AbsoluteFill>
  );
};
...
const Intro = () => {
  ...
  return (
    <AbsoluteFill>
      <Audio src={staticFile("sfx/vo-intro-capital.wav")} volume={0.95} />
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", gap: 26 }}>
        ...
        <div style={{ fontFamily: font, fontWeight: 900, fontSize: 80, color: "#fff", letterSpacing: 2 }}>GUESS THE <span style={{ color: GAME.gold }}>CAPITAL</span></div>
        <div style={{ display: "flex", gap: 16 }}>
          <span style={{ fontFamily: font, fontWeight: 800, fontSize: 34, color: GAME.blueDeep, background: GAME.gold, padding: "8px 26px", borderRadius: 999 }}>100 COUNTRIES</span>
...
const build = (items) => {
  ...
  for (const it of items) {
    ...
    num += 1;
    segs.push({ t: "round", item: it, num, from: f, dur: LT.round }); f += LT.round;
  }
    ...
    <AbsoluteFill style={{ backgroundColor: GAME.blueDeep }}>
      ...
      {segs.map((s, i) => (
        <Sequence key={i} from={s.from} durationInFrames={s.dur}>
          ...
          {s.t === "round" && <CapitalRound item={s.item} num={s.num} />}
          {s.t === "outro" && <Outro />}
        </Sequence>
      ))}
    </AbsoluteFill>
...
export const CAPITAL2_FRAMES = build(CAPITALS2).total;
export const CapitalQuiz2GS = () => <Comp items={CAPITALS2} />;
